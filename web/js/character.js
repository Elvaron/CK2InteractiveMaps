function Character(name, id) {
	...
	this.origin = 0;
	this.customTitle = "";
	this.rulerType = 0; // 1 - castle, 2 - city, 3 - temple
...
Character.prototype.needsRegent = function () {
	return (this.age < 16) || this.traits.includes(32);
}

Character.prototype.sanityCheck = function () {
	var requiresReparse = false;

	this.name = this.name.trim();

	var primaryTitle = (this.primaryTitle != 0 && titles.has(this.primaryTitle)) ? titles.get(this.primaryTitle) : null;

	// Regent
	if (primaryTitle != null && this.needsRegent() && (this.regent == 0 || (characterMap.has(this.regent) && !characterMap.get(this.regent).alive)))
	{
		// Create random regent
		var randomRegent = initSampleCharacter(false);
		this.regent = randomRegent.id;
	}

	// Council
	Object.setPrototypeOf(this.council, Council.prototype);
	if (primaryTitle != null && primaryTitle.titleImportance >= 2 && !this.council.exists())
	{
		randomCouncil(this);
	}

	// Liege
	if (this.liege == 0 && primaryTitle != null)
	{
		if (primaryTitle.parent != 0 && titles.has(primaryTitle.parent)) {
			this.liege = this.id;
			var searchTitle = primaryTitle;
			while (this.liege == this.id)
			{
				if (searchTitle.parent == 0 || !titles.has(searchTitle.parent))
				{
					this.liege = 0;
				} else  {
					var titleParent = titles.get(searchTitle.parent);
					this.liege = getTitleOwner(titleParent);
				}
			}
		}
	}
	...
	// Heir
	if (primaryTitle != null && this.heir == 0)
	{
		// This is Gilead specific!
		if (primaryTitle.titleImportance <= 2)
		{
			// Counts and below revert to King Agilias
			this.heir = 1;
		} else {
			// Dukes and above are inherited by Agnatic-Cognatic Primogeniture
			if (this.children.length > 0)
			{
				var eldestSon = 0;
				var eldestSonAge = 0;
				var eldestDaughter = 0;
				var eldestDaughterAge = 0;
				for (var i = 0; i < this.children.length; i++)
				{
					...
					if (child.gender == gender.FEMALE && child.age > eldestDaughterAge)
					{
						// New eldest daughter found
						eldestDaughterAge = child.age;
						eldestDaughter = child.id;
					}
					else if (child.gender == gender.MALE && child.age > eldestSonAge)
					{
						// New eldest son found
						eldestSonAge = child.age;
						eldestSon = child.id;
					}
				}
				...
				else if (eldestDaughter > 0)
				{
					this.heir = eldestDaughter;
				}
				...
				if (this.heir == 0)
				{
					...
					var eldestBrother = 0;
					var eldestBrotherAge = 0;
					var eldestSister = 0;
					var eldestSisterAge = 0;
					for (var i = 0; i < siblings.length; i++)
					{
						...
						if (sibling.gender == gender.FEMALE && sibling.age > eldestSisterAge)
						{
							// New eldest sister found
							eldestSisterAge = sibling.age;
							eldestSister = sibling.id;
						}
						else if (sibling.gender == gender.MALE && sibling.age > eldestBrotherAge)
						{
							// New eldest brother found
							eldestBrotherAge = sibling.age;
							eldestBrother = sibling.id;
						}
					}

					if (eldestBrother > 0)
					{
						this.heir = eldestBrother;
					}
					else if (eldestSister > 0)
					{
						this.heir = eldestSister;
					}
				}

				if (this.heir == 0 && this.liege > 0)
				{
					this.heir = this.liege;
				}
			}
		}
	}

	if (this.house == 0 && this.parents.length > 0)
	{
		// Parents might have a house
		for (var i = 0; i < this.parents.length; i++)
		{
			var parentId = this.parents[i];
			if (characterMap.has(parentId))
			{
				var parent = characterMap.get(parentId);
				if (parent.house > 0)
				{
					this.house = parent.house;
				}
			}
		}
	}

	// Child birth anachronistic
	for (var i = 0; i < this.children.length; i++)
	{
		...
		if (characterMap.has(childId))
		{
			...
			if (this.age > (child.age + 13) || !this.alive || !child.alive)
			{
				continue;
			}
			...
			if (child.age > this.age || child.age < 14)
			{
				// can't reasonably make child younger
				this.age = child.age + getRandomNumber(14, 20);
				if (this.age > 90)
				{
					this.alive = false;
				}
			...
			{
				child.age = Math.max(1, this.age - 13);
			}
		}
	}
	...
	if (!this.alive)
	{
		...
		{
			var titleId = this.titles[i];
			if (titles.has(titleId))
			{
				var title = titles.get(titleId);
				if (getTitleOwner(title) == this.id)
				{
					// Title needs to be inherited
					if (this.heir > 0 && characterMap.has(this.heir))
					{
						var heir = characterMap.get(this.heir);
						var makePrimary = (this.primaryTitle == titleId);
						if (heir.primaryTitle > 0 && titles.has(heir.primaryTitle))
						{
							if (titles.get(heir.primaryTitle).titleImportance >= title.titleImportance)
							{
								makePrimary = false;
							}
						}
						heir.addTitle(titleId, makePrimary);
						setTitleOwner(title, this.id);
					}
				}
			}
		}
...
Character.prototype.getRulerType = function () {
	if (this.hasOwnProperty('rulerType'))
	{
...
Character.prototype.addTitle = function (id, makePrimaryTitle) {
	...
	if (titles.has(id))
	{
		...
		if (title.titleImportance == 1 && makePrimaryTitle && this.rulerType == 0)
		{
		...
		if (title.titleImportance == 1 && title.parent > 0 && titles.has(title.parent) && titles.get(title.parent).primaryHolding == id)
		{
			this.titles.push(title.parent);
			if (makePrimaryTitle)
			{
				this.primaryTitle = title.parent;
			}
		}
	}
}
...
function getTitleName(character) {
	if (character.customTitle != "")
	{
		return character.customTitle;
	}
...
function getFullNameWithPrimaryTitleLocation(character) {
	var result = "";
	...
	if (character.house > 0 && houses.has(character.house))
	{
		 result = result + " " + houses.get(character.house).name;
	}
	...
	if (character.primaryTitle != 0 && titles.has(character.primaryTitle))
	{
		...
		if (primaryTitle.titleImportance == 2)
		{
			primaryTitle = titles.get(primaryTitle.primaryHolding);
		}

		result = result + " of " + primaryTitle.name;
	}
