		...
		if (title.primaryHolding > 0 && titles.has(title.primaryHolding))
		{
			...
			for (var i = 0; i < title.holdings.length; i++)
			{
				...
				if (titles.has(holdingId))
				{
					...
					$("[name=title_emblem" + holdingIterator + "]").attr("src", getHoldingTypePreviewImage(primaryHolding.holdingType));
				}
...
function saveTitle()
{
	...
	if (isProvince(title))
	{
		...
		{
			var holdingId = parseInt($("[name=HoldingId" + holdingIterator + "]").val());
			holdingMap.set(holdingId, holdingIterator);
...
function initTitlesForm()
{
	...
	$("#HoldingType1").change(function() {
		...
		$("[name=title_emblem1]").attr("src", getHoldingTypeImage(type));
	});
	$("#HoldingType2").change(function() {
		var type = $("#HoldingType2").val();
		$("[name=title_emblem2]").attr("src", getHoldingTypeImage(type));
	});
	$("#HoldingType3").change(function() {
		var type = $("#HoldingType3").val();
		$("[name=title_emblem3]").attr("src", getHoldingTypeImage(type));
	});
	$("#HoldingType4").change(function() {
		var type = $("#HoldingType4").val();
		$("[name=title_emblem4]").attr("src", getHoldingTypeImage(type));
	});
	$("#HoldingType5").change(function() {
		var type = $("#HoldingType5").val();
		$("[name=title_emblem5]").attr("src", getHoldingTypeImage(type));
	});
	$("#HoldingType6").change(function() {
		var type = $("#HoldingType6").val();
		$("[name=title_emblem6]").attr("src", getHoldingTypeImage(type));
	});
	$("#HoldingType7").change(function() {
		var type = $("#HoldingType7").val();
		$("[name=title_emblem7]").attr("src", getHoldingTypeImage(type));
	});
